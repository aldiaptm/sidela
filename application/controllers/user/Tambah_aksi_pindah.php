...
class Tambah_aksi_pindah extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('user/Berpindahtempat_model'); // Memuat model datameninggal_model
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect('user/tambahbt');
        } else {
            // Ambil nama dari input form
            $nama = $this->input->post('nama');

            // Cari id_penduduk berdasarkan nama
            $id_penduduk = $this->Berpindahtempat_model->get_id_penduduk_by_name($nama);
            ...
            if ($id_penduduk) {
                ...
                $insert_status = $this->Berpindahtempat_model->insert_data($data);
