...
class Kriteria extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/kriteria/tabel', $data);
    }
    ...
    public function ubah($id_kriteria = '')
    {
        ...
        if (empty($data['kriteria'])) {
            ...
            $this->form_validation->set_rules('kode_kriteria', 'Kode Kriteria', 'required|callback_cek_unik_kode_kriteria');
            $this->form_validation->set_rules('nama_kriteria', 'Nama Kriteria', 'required');
            $this->form_validation->set_rules('tipe', 'Tipe', 'required');
            ...
            if ($this->form_validation->run()) {
                ...
                $this->session->set_flashdata('pesan', '<div class="alert alert-success" role="alert">Data berhasil disimpan</div>');
                redirect('admin/kriteria/ubah/' . $id_kriteria);
            } else {
    ...
    public function cek_unik_kode_kriteria($kode_kriteria)
    {
        $query = $this->kriteria_model->cek_unik_kode_kriteria($kode_kriteria, $this->input->post('kode_kriteria_awal'));
        if ($query->num_rows() > 0) {
            $this->form_validation->set_message('cek_unik_kode_kriteria', '{field} sudah digunakan');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function prioritas()
    {
        ...
        $this->load->view('admin/kriteria/prioritas', $data);
    }
    ...
    public function reset()
    {
        ...
        redirect('admin/kriteria/prioritas');
    }

    // --- metode AHP --- START
    // menghitung matriks
    public function ahp_get_matrik_kriteria($kriteria) //hitung matriks kriteria
    {
        $matrik = array();
        $i = 0;
    ...
    //
    public function ahp_get_jumlah_kolom($matrik) //untuk menghitung jumlah kolom
    {
        $jumlah_kolom = array();
    ...
    public function ahp_get_normalisasi($matrik, $jumlah_kolom) // perbandingan  berpasangan
    {
    ...
    public function ahp_get_prioritas($matrik_normalisasi) //normalisasi u/ prioritas
    {
        $prioritas = array();
    ...
    public function ahp_get_matrik_baris($prioritas, $matrik_kriteria) //menghitung matriks baris
    {
        $matrik_baris = array();
    ...
    public function ahp_get_jumlah_matrik_baris($matrik_baris) //perhitungan rasio konsistensi
    {
        $jumlah_baris = array();
    ...
    public function tampil_data_1($matrik_kriteria, $jumlah_kolom)
    {
        ...
        $list_data = '';
        $list_data .= '<tr><td class="text-center">Kriteria</td>';
        ...
        foreach ($kriteria as $row) {
            $list_data .= '<tr>';
            $list_data .= '<td style="text-align: left">' . $row->nama_kriteria . '</td>';
            $ii = 0;
            foreach ($kriteria as $row2) {
    ...
    public function tampil_data_2($matrik_normalisasi, $prioritas)
    {
        ...
        $list_data2 = '';
        $list_data2 .= '<tr><td class="text-center">Kriteria</td>';
        foreach ($kriteria as $row) {
        ...
        foreach ($kriteria as $row) {
            $list_data2 .= '<tr>';
            $list_data2 .= '<td style="text-align: left">' . $row->nama_kriteria . '</td>';
            $jumlah = 0;
            $ii = 0;
            foreach ($kriteria as $row2) {
    ...
    public function tampil_data_3($matrik_baris, $jumlah_matrik_baris)
    {
        ...
        $list_data3 = '';
        $list_data3 .= '<tr><td class="text-center">Kriteria</td>';
        foreach ($kriteria as $row) {
        ...
        foreach ($kriteria as $row) {
            $list_data3 .= '<tr>';
            $list_data3 .= '<td style="text-align: left">' . $row->nama_kriteria . '</td>';
            $ii = 0;
            foreach ($kriteria as $row2) {
    ...
    public function tampil_data_4($jumlah_matrik_baris, $prioritas, $hasil_tabel_konsistensi)
    {
        ...
        foreach ($kriteria as $row) {
            $list_data4 .= '<tr>';
            $list_data4 .= '<td style="text-align: left">' . $row->nama_kriteria . '</td>';
